...
static PerfStats run_ecb(size_t total_blocks) {
    ...
    for (size_t processed = 0; processed < total_blocks; processed += CHUNK_BLOCKS) {
        ...
        CUDA_CHECK(cudaEventRecord(start_event));
        chacha_ecb_kernel<<<blocks, THREADS_PER_BLOCK>>>(d_input, d_output, current_blocks);
...
static PerfStats run_ctr(size_t total_blocks, uint64_t ctr) {
    ...
    for (size_t processed = 0; processed < total_blocks; processed += CHUNK_BLOCKS) {
        ...
        CUDA_CHECK(cudaEventRecord(start_event));
        chacha_ctr_kernel<<<blocks, THREADS_PER_BLOCK>>>(d_output, current_blocks, ctr + processed);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaEventRecord(stop_event));
        CUDA_CHECK(cudaEventSynchronize(stop_event));
        float ms = 0.0f;
...
int main(int argc, char **argv) {
    ...
    const uint64_t ctr = 0x1020304050607080ULL;

    printf("ChaCha20 GPU ECB\n");
