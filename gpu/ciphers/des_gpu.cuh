...
inline constexpr CipherDescriptor descriptor = {"des", "DES", 8};
...
__device__ __forceinline__ uint64_t encrypt_block(uint64_t block, const uint64_t round_keys[DES_ROUNDS]) {
...
__global__ void ecb_kernel(const uint8_t *input, uint8_t *output, uint64_t key, size_t blocks) {
    ...
    for (size_t idx = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; idx < blocks; idx += stride) {
        ...
        const uint64_t block = load_be64(input + offset);
        store_be64(output + offset, encrypt_block(block, round_keys));
    }
}

__global__ void ctr_kernel(uint8_t *output, uint64_t key, size_t blocks, uint64_t ctr) {
    const size_t stride = static_cast<size_t>(blockDim.x) * gridDim.x;
    uint64_t round_keys[DES_ROUNDS];
    generate_round_keys(key, round_keys);
    for (size_t idx = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; idx < blocks; idx += stride) {
        const size_t offset = idx * 8;
        store_be64(output + offset, encrypt_block(ctr + idx, round_keys));
    }
}
...
inline cudaError_t launch_ecb(const uint8_t *d_input, uint8_t *d_output, size_t blocks) {
    ...
    ecb_kernel<<<grid, GPU_THREADS_PER_BLOCK>>>(d_input, d_output, 0x133457799BBCDFF1ULL, blocks);
    return cudaGetLastError();
}

inline cudaError_t launch_ctr(uint8_t *d_output, size_t blocks, uint64_t ctr) {
    const int grid = gpu_grid_for_blocks(blocks);
    ctr_kernel<<<grid, GPU_THREADS_PER_BLOCK>>>(d_output, 0x133457799BBCDFF1ULL, blocks, ctr);
    return cudaGetLastError();
}
