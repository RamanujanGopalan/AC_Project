 ...
 *   cipher_kalyna_setup (key)              — key expansion + __constant__ upload
 *   cipher_kalyna_launch(d_in, d_out, ...) — kernel launch
 */
...
void cipher_kalyna_launch(uint8_t *d_in, uint8_t *d_out, int blocks, int threads) {
    ...
    kalyna_encrypt_kernel<<<grid, threads>>>(d_in, d_out, blocks);
}
// ── CTR ──────────────────────────────────────────────────────────────────────
...
void cipher_kalyna_launch_ctr(uint8_t *d_in, uint8_t *d_out,
                               int blocks, int threads,
                               uint64_t nonce, uint64_t counter_start){
    int grid = (blocks + threads - 1) / threads;
    kalyna_ctr_kernel<<<grid, threads>>>(d_out, d_in, blocks, nonce, counter_start);
}
