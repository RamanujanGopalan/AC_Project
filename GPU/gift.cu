...
void cipher_gift_launch(uint8_t *d_in, uint8_t *d_out, int blocks, int threads) {
    ...
    gift_ecb_kernel<<<grid, threads>>>(d_out, d_in, blocks);
}
...
void cipher_gift_launch_ctr(uint8_t *d_in, uint8_t *d_out,
                             int blocks, int threads,
                             uint64_t nonce, uint64_t counter_start){
    int grid = (blocks + threads - 1) / threads;
    gift_ctr_kernel<<<grid, threads>>>(d_out, d_in, blocks, nonce, counter_start);
}