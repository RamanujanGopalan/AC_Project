...
#define NUM_BLOCKS  (100 * 1024 * 1024)
...
void cipher_aes_launch_ctr(uint8_t *d_in, uint8_t *d_out, int blocks, int threads,
                            uint64_t nonce, uint64_t ctr_start);

void cipher_kalyna_setup    (const uint8_t *key);
void cipher_kalyna_launch_ctr(uint8_t *d_in, uint8_t *d_out, int blocks, int threads,
                               uint64_t nonce, uint64_t ctr_start);

void cipher_simon_setup    (const uint8_t *key);
void cipher_simon_launch_ctr(uint8_t *d_in, uint8_t *d_out, int blocks, int threads,
                              uint64_t nonce, uint64_t ctr_start);

void cipher_gift_setup    (const uint8_t *key);
void cipher_gift_launch_ctr(uint8_t *d_in, uint8_t *d_out, int blocks, int threads,
                             uint64_t nonce, uint64_t ctr_start);
...
struct CipherDesc {
    ...
    void      (*launch_ctr) (uint8_t *d_in, uint8_t *d_out, int blocks, int threads,
                             uint64_t nonce, uint64_t ctr_start);
};
...
static const CipherDesc ciphers[] = {
    {
        ...
        cipher_aes_setup, cipher_aes_launch_ctr
    },
    {
        "kalyna",
        {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
         0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f},
        cipher_kalyna_setup, cipher_kalyna_launch_ctr
    },
    {
        "simon",
        {0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,
         0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07},
        cipher_simon_setup, cipher_simon_launch_ctr
    },
    {
        "gift",
        {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
         0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f},
        cipher_gift_setup, cipher_gift_launch_ctr
    },
};
...
static void run_benchmark_ctr(const CipherDesc &c)
{
    const size_t size = (size_t)NUM_BLOCKS * BLOCK_BYTES;

    // Use a fixed nonce and start counter for reproducibility
    uint64_t nonce         = 0xDEADBEEFCAFEBABEULL;
    uint64_t counter_start = 0;

    printf("\n===== Cipher: %s (CTR) =====\n", c.name);
    printf("Nonce: %016llx  Counter start: %llu\n\n",
           (unsigned long long)nonce, (unsigned long long)counter_start);
    ...
    c.setup(c.default_key);
    cudaMemcpy(d_in, h_in, size, cudaMemcpyHostToDevice);
    cudaEventRecord(h2dStop);
    cudaEventSynchronize(h2dStop);

    // Warm-up
    c.launch_ctr(d_in, d_out, NUM_BLOCKS, THREADS, nonce, counter_start);
